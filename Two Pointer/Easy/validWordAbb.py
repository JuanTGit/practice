...
def validWordAbbreviation(word, abbr):
    ...
    while wordPointer < len(word) and abbrPointer < len(abbr):
        if word[wordPointer] != abbr[abbrPointer] and abbr[abbrPointer].isnumeric():
            wordPointer += int(abbr[abbrPointer])
            abbrPointer += 1
            if word[abbrPointer] != abbr[abbrPointer]:
                return False
        elif word[abbrPointer] != abbr[abbrPointer]:
            return False
        wordPointer += 1
        abbrPointer += 1
    return True
print(validWordAbbreviation('international', 'i9l'))